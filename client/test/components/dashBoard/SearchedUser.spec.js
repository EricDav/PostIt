...
describe('SearchUser Component', () => {
  it('should render SearchUser component', () => {
    ...
    expect(wrapper.node.props.className)
      .toEqual('collection-item avatar email-unread group-channel');
    expect(wrapper.node.type).toEqual('li');
    expect(wrapper.node.props.children[0].props.children[0].type)
      .toEqual('span');
    expect(wrapper.node.props.children[0].props.children[0].props.className)
      .toEqual('group-title');
  });
});
