...
import { SET_CURRENT_GROUP_MEMBERS, DELETE_GROUP, SET_CURRENT_USER_GROUPS, ADD_GROUP }
...
describe('Group action', () => {
  it('creates ADD_GROUP  when a user create a group', () => {
    const response = {
      data: { group: {
        id: 1,
  ...
  it(`creates SET_CURRENT_USER_GROUPS when a user request for all groups`, () => {
    const response = {
      data: { groups: [{
        id: 1,
        name: 'Mathematics world',
  ...
  it(`creates SET_CURRENT_USER_GROUPS when a user request to see all members`, () => {
    const response = {
      data: [{
        id: 1,
  ...
  it('creates DELETE_GROUP  when a user DELETE a group', () => {
    ...
    const expectedActions = [{ groupData: { description: 'This where we solve maths', id: 1, name: 'Mathematics world' }, type: DELETE_GROUP }];
  ...
  it('creates UPDATE_GROUP_DATA, DELETE_CURRENT_GROUP_DATA  when a user update a group', () => {
    axios.delete = jest.fn(() => Promise.resolve(respnose));
    const response = {
      name: 'English world',
      description: 'This where we solve maths'
    ...
    const expectedActions = [{ type: 'UPDATE_GROUP_DATA',
      ...
      updateGroupData: { description: 'This where we solve maths', name: 'English world' } }];
    const store = mockStore({ groups: {} });
